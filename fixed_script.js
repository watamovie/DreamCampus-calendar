  ...
  try{
    ...
    for(var i=0; i<slots.length; i++){
        ...
        for(var j=0; j<subjectLinks.length; j++){
            ...
            // Primary ID: ..._hlSbjNm
            // Secondary ID: ..._hlSbjNm_double_X
            // We check if the ID contains "double"
            var isSecondary = subLink.id.indexOf("double") !== -1;

            // Only assign the slot description to the primary class.
            // The secondary class details are not visible in this view, so we leave it empty
            // to avoid incorrectly attributing the primary class's details to it.
    ...
    /*=== ④ Filter included classes (Same Name, Time Inclusion) ===*/
    function toMin(tStr) {
        if (!tStr) return 0;
    ...
    for (var i = 0; i < out.length; i++) {
        ...
        var A_start = toMin(A.start);
        var A_end = toMin(A.end);

        for (var j = 0; j < out.length; j++) {
            if (i === j) continue;
            var B = out[j];

            // Condition 1: Same Date and Same Summary
            if (A.date === B.date && A.summary === B.summary) {
                var B_start = toMin(B.start);
                var B_end = toMin(B.end);

                // Condition 2: B includes A (B starts before/at A, B ends after/at A)
                // B: |-----------|
                // A:    |-----|
                if (B_start <= A_start && B_end >= A_end) {
                    var A_duration = A_end - A_start;
                    var B_duration = B_end - B_start;

                    // Condition 3: Disable shorter (A) - shadowed by B
                    if (B_duration > A_duration) {
                        isShadowed = true;
                        break;
                    }
                    // If exact same time (duplicate), keep the first one found
                    if (B_duration === A_duration && j < i) {
                        isShadowed = true;
                        break;
                    }
                }
